...
PROJECT = Path(os.environ.get("ATTNRES_SOURCE_DIR", "/workspace/project")).expanduser().resolve()
...
if FLA_AVAILABLE:
    ...
    _serialized_fla_preflight = os.environ.get("ATTNRES_FLA_HOST_PREFLIGHT", "").strip()
    _FLA_HOST_PREFLIGHT = (
        json.loads(_serialized_fla_preflight)
        if _serialized_fla_preflight
        else fla_checkout_metadata(PROJECT, FLA)
    )
else:
...
TORCH_VERSION = os.environ.get("ATTNRES_TORCH_VERSION", "2.13.0")
TRITON_VERSION = os.environ.get("ATTNRES_TRITON_VERSION", "3.7.1")
if TORCH_VERSION != "2.13.0" or TRITON_VERSION != "3.7.1":
    raise ValueError("current 24-layer campaign requires Torch 2.13.0 and Triton 3.7.1")
...
if FLA_AVAILABLE:
    _fla_revision = str(_FLA_HOST_PREFLIGHT["revision"])
    image = image.run_commands(
        "git clone --quiet https://github.com/fla-org/flash-linear-attention.git /workspace/fla",
        f"git -C /workspace/fla checkout --quiet --detach {_fla_revision}",
    )
# Runtime source mounts must remain the final image layers.
image = (
    image
...
def _run(payload, expected):
    ...
    try:
        ...
        for name, digest in manifest.items():
            ...
            if name.startswith("benchmarks/") and payload["task"] not in {"training", "training_graph", "suite", "suite_campaign", "source_profile"}:
                continue
        ...
        if payload["task"] == "preflight":
            ...
            preflight_checks = {"compiled_bf16_autograd": True, "gluon_import": gluon is not None}
            if FLA_AVAILABLE:
                preflight_checks["fla_git_revision"] = subprocess.check_output(
                    ["git", "-C", "/workspace/fla", "rev-parse", "HEAD"], text=True
                ).strip()
                preflight_checks["fla_git_clean"] = not bool(subprocess.check_output(
                    ["git", "-C", "/workspace/fla", "status", "--porcelain", "--untracked-files=all"], text=True
                ).strip())
            result["checks"] = preflight_checks
            ...
            if result["measurements"]["status"] == "failed":
                result["status"] = "failed"
        elif payload["task"] == "suite_campaign":
            from benchmarks.run import run_suite
            configs = payload.get("configs")
            if not isinstance(configs, list) or not configs:
                raise ValueError("suite_campaign requires a nonempty configs list")
            campaign_results = []
            for benchmark_config in configs:
                seed = benchmark_config.get("seed")
                campaign_result = run_suite(benchmark_config)
                model = campaign_result.get("model_timings", {})
                model_complete = (
                ...
                campaign_results.append({
                    "seed": seed,
                    "compiled_step_execution_status": "complete" if model_complete else "failed",
                    "measurements": campaign_result,
                })
...
_FUNCTION_OPTIONS = {
    "cpu": 8, "memory": 65536, "timeout": 10800,
...
def main(gpu: str = "H100!", task: str = "preflight", config: str = "{}", output: str = ""):
    ...
    payload = {"task": task, "config": config_value}
    if task == "suite_campaign":
        import copy
        seeds = (20260827, 20260903, 20260911)
        configs = []
        for seed in seeds:
            item = copy.deepcopy(config_value)
            item["seed"] = seed
            item["compiled_step_campaign"]["seed"] = seed
            configs.append(item)
