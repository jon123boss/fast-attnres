...
def training_checkpoint(output, cache_archive=None, commit=None):
    ...
    def save(report):
        ...
        if cache_archive and qualified - saved_arms:
            try:
...
def main():
    ...
    if args.cache_archive:
        if not args.cache_volume:
            parser.error("--cache-archive requires --cache-volume")
        import modal
    ...
    run_training(json.loads(args.config.read_text()),
                 training_checkpoint(args.output, args.cache_archive, commit))
